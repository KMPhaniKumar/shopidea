...
**Code lives in `reelmart/`; infra-as-code in `reelmart-infra/infra/terraform/`. Read these before starting work:**
...
Nested `CLAUDE.md` files add local context automatically in `reelmart/services/`, `reelmart-infra/infra/terraform/`, and `reelmart/apps/web/`.
...
- **Backend hosting**: **AWS ECS Fargate** (cluster `reelmart-dev`, ap-south-1), images in ECR, behind ALB `api-dev.reelmart.in`. Infra in **Terraform** (`reelmart-infra/infra/terraform/`) — change infra there, not via raw AWS CLI.
